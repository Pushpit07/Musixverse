...
const Musixverse = artifacts.require("./Musixverse.sol");

require("chai").use(require("chai-as-promised")).should();

contract("Musixverse", ([deployer, seller, buyer, buyer2]) => {
	let contract;

	before(async () => {
		contract = await Musixverse.deployed();
	});
	...
	describe("songs", async () => {
		describe("minting", async () => {
			it("creates a new song", async () => {
				...
				// FAILURE: Cannot mint song having same name twice
				await contract.createSong(
					"Yay!",
					"Sommaiya Angrish",
					web3.utils.toWei("2", "Ether"),
		...
		describe("purchase", async () => {
			...
			it("purchases song when artist is also the current owner", async () => {
				...
				assert.equal(event.id.toNumber(), songId + 1, "id is correct");
			...
			it("purchases song when artist and owner are different", async () => {
				...
				// Track MusixverseMain balance before purchase
				let musixverseMainAddress = "0x159507b2b3829791fAB794581D2aC074F3596013";
				let oldMusixverseMainBalance = await web3.eth.getBalance(musixverseMainAddress);
				oldMusixverseMainBalance = BigNumber(oldMusixverseMainBalance);
				...
				let newMusixverseMainBalance = await web3.eth.getBalance(musixverseMainAddress);
				newMusixverseMainBalance = BigNumber(newMusixverseMainBalance);
				...
				let expectedPlatformBalance = oldMusixverseMainBalance.plus(priceTransferredToPlatform);
				assert.equal(newMusixverseMainBalance.toString(), expectedPlatformBalance.toString());

				// Failure: Tries to buy a song that does not exist- Must have valid id
				await contract.purchaseSong(99, { from: buyer2, value: web3.utils.toWei("0.2", "Ether") }).should.be
					.rejected;
				// Failure: Tries to buy without enough ether
				await contract.purchaseSong(songsCount - 1, { from: buyer2, value: web3.utils.toWei("0.1", "Ether") })
					.should.be.rejected;
				// Failure: Buyer tries to buy again- The same address can't be both the buyer and the seller
				await contract.purchaseSong(songsCount - 1, { from: buyer2, value: web3.utils.toWei("0.5", "Ether") })
					.should.be.rejected;
			});
		...
		describe("toggleOnSale", async () => {
			...
			it("toggles the onSale attribute", async () => {
				let _song = await contract.songs(songsCount - 1);

				//Success: Current owner toggles the onSale attribute
				const result = await contract.toggleOnSale(songsCount - 1, { from: buyer2 });
				...
				await contract.toggleOnSale(songsCount - 1, { from: seller }).should.be.rejected;
				await contract.toggleOnSale(songsCount - 1, { from: deployer }).should.be.rejected;
			});
		});
		...
		describe("updatePrice", async () => {
			...
			it("updates the Song NFT Price", async () => {
				//Success: Current owner toggles the onSale attribute
				const result = await contract.updatePrice(songsCount - 1, web3.utils.toWei("1.5", "Ether"), {
					from: buyer2,
				});
				...
				await contract.updatePrice(songsCount - 1, { from: seller }).should.be.rejected;
				await contract.updatePrice(songsCount - 1, { from: deployer }).should.be.rejected;
			});
		});
	});
});
