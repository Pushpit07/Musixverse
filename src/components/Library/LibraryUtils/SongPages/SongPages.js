			...
			<div className="song_card_component" key={i}>
				<Link to={`/song-info/${props.songNFTs[i].id}`}>
					<div className="library_cards_cover_div justify-content-center align-items-center">
						<img src={`https://ipfs.infura.io/ipfs/${props.songNFTs[i].imgHash}`} alt="song cover" />
	...
	for(let i = 0; i < numberOfPages; i++) {
		pagerBar.push(
			<li
				...
				className={
					"page-item" +
					(currentPage === i + 1 ? " current-page-item" : "") +
					(Math.floor(i / 4) !== interval ? " hide-item" : "")
				}
			>
						...
						<nav className="paging-bar" aria-label="Page navigation example">
							<ul className="pagination">
								...
								<li className="page-item">
									<button
										className="page-link pagination_prev_next"
